<script>
    import {currentTemplate, attach, fileselect} from './store.ts';
    ...
    import {dndzone} from "svelte-dnd-action";
        ...
        // let unlisten = await listen('progress', event => {
    ...
    // let promise = progress();
    // onDestroy(() => {
    //     $currentTemplate.destroy = true;
    //     if ($currentTemplate.unlisten) {
    //         console.log('the component is being destroyed');
    //         $currentTemplate.unlisten();
    //     }
    // });
...
<div class="grid">
    ...
    {:else}
        <!--{#await promise then value}-->
            <div use:dndzone="{{items: $currentTemplate.files, flipDurationMs}}" on:consider="{handleDndConsider}" on:finalize="{handleDndFinalize}" class="flex flex-col rounded-lg">
                {#each $currentTemplate.files as file(file.filename)}
                    <div class="shadow-sm rounded-lg" animate:flip="{{duration: flipDurationMs}}">
                        <Progress fullname={file.filename} name={file.name} complete={file.complete} progress={file.progress} speed={file.speed}/>
                    </div>
                {/each}
            </div>
        <!--{/await}-->

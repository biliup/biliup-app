<script lang="ts">
    ...
    import {createPop, getManuscriptInfo} from "./common";
	...
	async function add() {		
		...
		const result = await getManuscriptInfo(tempName,$template)
				...
				<div slot="box" let:componentId>
					<div class="form-control w-full">
						<label class="label">
							...
							<a
								href="http://www.bilibili.com/video/BV1ip4y1x7Gi"
								target="_blank"
								style="color: blue;"
							>
								<span>点我观看视频教程</span>
						...
						<label class="label">
							<span class="label-text">输入其他将新建投稿模板</span>
						</label>
						<input
							type="text"
							bind:value={tempName}
							placeholder={'未命名模板' + Object.keys($template).length}
							class="input input-bordered w-full"
						/>
					</div>

					<div class="modal-action" style="justify-content: space-around;">
                        <label for={componentId} class="btn" style="background-color: #dc2626; color: black; border: none;">关闭</label>
						<label for={componentId} on:click={add} class="btn btn-accent">
							添加模板
						</label>
					</div>
				</div>
